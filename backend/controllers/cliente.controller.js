...
//obtener clientes y su historial de prestamos en el rol de administrador
...
async function obtenerInfoPrestamosClientesSupervisor(req, res) {
  ...
  try {
    ...
    const [clientes] = await conn.query(
      ...
      WHERE s.id_usuario = ?   -- 🔥 acá solo por supervisor
      ORDER BY c.nombre, c.apellido`,
      [id_usuario] 
    );
    ...
    if (!clientes.length) {
      return res.status(200).json({
        clientes: [],
        message: "No se encontraron clientes para este supervisor"
      });
    }
    ...
    console.error("Error en listarClientesConPrestamosSupervisor:", error);
    return res.status(500).json({
      error: "SERVER_ERROR",
      message: "Error interno del servidor"
    });
